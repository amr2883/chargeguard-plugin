 ...
 * `Version:` line. Used to cache-bust every enqueued script/style أ¢â‚¬â€‌
 ...
 * ChargeGuard_Admin_Settings::settings_page()). This is a PUBLIC value أ¢â‚¬â€‌
 * safe to ship in HTML أ¢â‚¬â€‌ unlike chargeguard_api_key / webhook/signing
 ...
 *
 * ChargeGuard never defines this itself أ¢â‚¬â€‌ it must be set by the
 ...
 * (custom order tables). Must run on before_woocommerce_init أ¢â‚¬â€‌ the hook
 * WooCommerce itself fires specifically for this declaration, ahead of
 * plugins_loaded (where chargeguard_init() runs) أ¢â‚¬â€‌ so this registration
...
// chargeguard_add_device_fingerprint_to_order() removed â€” its sole
...
// function silently never ran for Blocks-checkout orders â€” exactly the
...
if (!function_exists('chargeguard_add_fingerprint_to_webhook_payload')) {
    function chargeguard_add_fingerprint_to_webhook_payload($payload, $resource, $resource_id, $webhook_id) {
        ...
        if ($resource === 'order' && isset($payload['id'])) {
            // Canonical accessor â€” see ChargeGuard_Dynamic_Firewall::get_order_device_fp()
            ...
            // is a NEW top-level field â€” `_chargeguard_device_token` is a
            ...
            // no token â€” pre-device-token-feature orders, older plugin
...
function chargeguard_maybe_show_deactivated_notice() {
    ...
    if (!$connected) {
        // Store was never connected أ¢â‚¬â€‌ nothing is still transmitting, no need to warn.
    ...
    <div class="notice notice-warning is-dismissible">
        <p>
            <strong>ChargeGuard has been deactivated</strong> أ¢â‚¬â€‌ but your store's
...
new ChargeGuard_PayPal_Webhook();
    ...
    // `_chargeguard_device_fp` â€” no separate hook needed here anymore.
...
// checkout request path â€” see ChargeGuard_Trusted_Proxy::refresh_cf_ranges().
