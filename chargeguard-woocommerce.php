 ...
 * `Version:` line. Used to cache-bust every enqueued script/style ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ
 ...
 * ChargeGuard_Admin_Settings::settings_page()). This is a PUBLIC value ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ
 * safe to ship in HTML ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ unlike chargeguard_api_key / webhook/signing
 ...
 *
 * ChargeGuard never defines this itself ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ it must be set by the
 ...
 * (custom order tables). Must run on before_woocommerce_init ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ the hook
 * WooCommerce itself fires specifically for this declaration, ahead of
 * plugins_loaded (where chargeguard_init() runs) ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ so this registration
...
// chargeguard_add_device_fingerprint_to_order() removed أ¢â‚¬â€‌ its sole
...
// function silently never ran for Blocks-checkout orders أ¢â‚¬â€‌ exactly the
...
if (!function_exists('chargeguard_add_fingerprint_to_webhook_payload')) {
    function chargeguard_add_fingerprint_to_webhook_payload($payload, $resource, $resource_id, $webhook_id) {
        ...
        if ($resource === 'order' && isset($payload['id'])) {
            // Canonical accessor أ¢â‚¬â€‌ see ChargeGuard_Dynamic_Firewall::get_order_device_fp()
            ...
            // is a NEW top-level field أ¢â‚¬â€‌ `_chargeguard_device_token` is a
            ...
            // no token أ¢â‚¬â€‌ pre-device-token-feature orders, older plugin
...
function chargeguard_maybe_show_deactivated_notice() {
    ...
    if (!$connected) {
        // Store was never connected ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ nothing is still transmitting, no need to warn.
    ...
    <div class="notice notice-warning is-dismissible">
        <p>
            <strong>ChargeGuard has been deactivated</strong> ط£آ¢أ¢â€ڑآ¬أ¢â‚¬â€Œ but your store's
...
new ChargeGuard_PayPal_Webhook();
    ...
    // `_chargeguard_device_fp` أ¢â‚¬â€‌ no separate hook needed here anymore.
...
// checkout request path أ¢â‚¬â€‌ see ChargeGuard_Trusted_Proxy::refresh_cf_ranges().
